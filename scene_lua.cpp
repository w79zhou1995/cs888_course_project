...
int gr_mesh_cmd(lua_State* L)
{
	...
	/*
	if (mesh_map.find( sfname ) == mesh_map.end()) {
	  auto i = mesh_map.find( "Assets/" + sfname );
	}
	*/

	if( i == mesh_map.end() ) {
	  std::cout << "looking for " << sfname << std::endl;
	  mesh = new Mesh( obj_fname );
	  if (mesh->get_verts() == 0) {
		  std::cout << "no luck" <<std::endl;
		  std::string dir("Assets/" + (std::string)obj_fname);
...
int gr_cloth_cmd(lua_State* L)
{
  ...
  const char* geometry = luaL_checkstring(L, 3);

  glm::vec3 center;
  get_tuple(L, 4, &center[0], 3);

  const float r = luaL_checknumber(L, 5);
  const float s = luaL_checknumber(L, 6);
    
  const int time = luaL_checknumber(L, 7);
  const int start = luaL_checknumber(L, 8);

  Cloth *cloth = nullptr;
  cloth = new Cloth( size, geometry, center, r, s, time, start );

  data->node = new GeometryNode( name, cloth );
...
// Create a Fractal using mesh
extern "C"
int gr_fractal_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
...
int gr_material_cmd(lua_State* L)
{
  ...
  double kr = luaL_checknumber(L, 4);
